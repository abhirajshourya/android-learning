...
class MainActivity : AppCompatActivity() {
    ...
    private fun clickLemonImage() {
        ...
        Log.i(TAG, "lemonadeState: $lemonadeState")
        when (lemonadeState) {
            SELECT -> {
            ...
            DRINK -> {
                ...
                Log.i(TAG, "lemonadeState: Drink complete")
            }
            else -> {
                lemonadeState = SELECT
                Log.i(TAG, "lemonadeState: Restart, Pick from Tree!")
            }
        }
        setViewElements()
    }
    ...
    private fun setViewElements() {
        ...
        when (lemonadeState) {
            SELECT -> {
                textValue = resources.getString(R.string.lemon_select)
                lemonImage?.setImageResource(R.drawable.lemon_tree)
            }
            SQUEEZE -> {
                textValue = resources.getString(R.string.lemon_squeeze)
                lemonImage?.setImageResource(R.drawable.lemon_squeeze)
            }
            DRINK -> {
                textValue = resources.getString(R.string.lemon_drink)
                lemonImage?.setImageResource(R.drawable.lemon_drink)
            }
            else -> {
                textValue = resources.getString(R.string.lemon_empty_glass)
                lemonImage?.setImageResource(R.drawable.lemon_restart)
            }
        }
        textAction.text = textValue
        lemonImage?.contentDescription = textValue
    }
